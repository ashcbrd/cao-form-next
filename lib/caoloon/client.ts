...
class CaoloonClient {
  ...
  constructor() {
    this.config = {
      baseUrl: process.env.CAOLOON_BASE_URL || "https://test.caoloon.com/v3",
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Caoloon API error: ${response.status} - ${errorData.message || response.statusText}`
        );
      }
  ...
  async searchOrganizations(
    params: CaoloonSearchParams
  ): Promise<CaoloonResponse<CaoloonOrganization[]>> {
    ...
    return this.makeRequest<CaoloonOrganization[]>(
  ...
  async getOrganizationByName(
    name: string
  ): Promise<CaoloonOrganization | null> {
    try {
